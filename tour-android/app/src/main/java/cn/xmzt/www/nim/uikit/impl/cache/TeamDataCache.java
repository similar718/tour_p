package cn.xmzt.www.nim.uikit.impl.cache;
...
public class TeamDataCache {
    ...
    private Observer<Team> teamRemoveObserver = new Observer<Team>() {
        ...
        public void onEvent(Team team) {
            ...
            EventBus.getDefault().post(new RefreshSchedulingListBus(1));//通知行程群列表刷新
        }
    };
    ...
    private Observer<List<TeamMember>> memberRemoveObserver = new Observer<List<TeamMember>>() {
        ...
        public void onEvent(List<TeamMember> member) {
            ...
            for(TeamMember teamMember:member){
                String groupId=teamMember.getTid();
                String userId=teamMember.getAccount();
                GroupUserInfo info = TourDatabase.getDefault(TourApplication.context).getGroupUserInfoDao().getData(groupId,userId);
                if (info != null){
                    TourDatabase.getDefault(TourApplication.context).getGroupUserInfoDao().delete(info);
                }
//                TourDatabase.getDefault(TourApplication.context).getGroupUserInfoDao().delete(groupId,userId); // 这个delete有问题 只删除了数据库中的group和userid
                UserBasicInfoBean user=TourApplication.getUser();
                if (user!=null&&groupId.equals(MsgExtensionType.groupId)&&userId.equals(user.getUserId())){
                    // 相除对讲相关业务
