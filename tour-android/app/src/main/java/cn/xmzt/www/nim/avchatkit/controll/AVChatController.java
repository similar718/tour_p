package cn.xmzt.www.nim.avchatkit.controll;
...
import cn.xmzt.www.nim.avchatkit.common.widgets.MultiSelectDialog;
import cn.xmzt.www.nim.avchatkit.config.AVChatConfigs;
import cn.xmzt.www.nim.avchatkit.config.AVPrivatizationConfig;
import cn.xmzt.www.nim.avchatkit.constant.AVChatExitCode;
import cn.xmzt.www.nim.avchatkit.constant.CallStateEnum;
import cn.xmzt.www.nim.avchatkit.module.AVChatControllerCallback;
import cn.xmzt.www.nim.avchatkit.module.AVSwitchListener;
...
public class AVChatController {
        ...
        AVChatManager.getInstance().call2(account, avChatType, notifyOption, new AVChatCallback<AVChatData>() {
            ...
            public void onFailed(int code) {
                ...
                if (code == ResponseCode.RES_FORBIDDEN) {
                    ToastUtils.showText(context, R.string.avchat_no_permission);
                } else {
                    ToastUtils.showText(context, R.string.avchat_call_failed);
                }
    ...
    public void receive(final AVChatType avChatType, final AVChatControllerCallback<Void> callback) {
        ...
        AVChatManager.getInstance().accept2(avChatData.getChatId(), new AVChatCallback<Void>() {
            ...
            public void onFailed(int code) {
                if (code == -1) {
                    ToastUtils.showText(context, "本地音视频启动失败");
                } else {
                    ToastUtils.showText(context, "建立连接失败");
                }
    ...
    public void showQuitToast(int code) {
        switch (code) {
            ...
            case AVChatExitCode.CONFIG_ERROR: // 服务器返回数据错误
                ToastUtils.showText(context, R.string.avchat_net_error_then_quit);
                break;
            case AVChatExitCode.REJECT:
                ToastUtils.showText(context, R.string.avchat_call_reject);
                break;
            case AVChatExitCode.PEER_HANGUP:
            case AVChatExitCode.HANGUP:
                if (isCallEstablish.get()) {
                    ToastUtils.showText(context, R.string.avchat_call_finish);
                }
                break;
            case AVChatExitCode.PEER_BUSY:
                ToastUtils.showText(context, R.string.avchat_peer_busy);
                break;
            case AVChatExitCode.PROTOCOL_INCOMPATIBLE_PEER_LOWER:
                ToastUtils.showText(context, R.string.avchat_peer_protocol_low_version);
                break;
            case AVChatExitCode.PROTOCOL_INCOMPATIBLE_SELF_LOWER:
                ToastUtils.showText(context, R.string.avchat_local_protocol_low_version);
                break;
            case AVChatExitCode.INVALIDE_CHANNELID:
                ToastUtils.showText(context, R.string.avchat_invalid_channel_id);
                break;
            case AVChatExitCode.LOCAL_CALL_BUSY:
                ToastUtils.showText(context, R.string.avchat_local_call_busy);
                break;
            default:
                break;
        }
