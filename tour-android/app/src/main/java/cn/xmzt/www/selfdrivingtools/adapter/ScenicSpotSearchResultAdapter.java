 ...
 * 智慧景区导览列表 Adapter
 */
public class ScenicSpotSearchResultAdapter extends RecyclerView.Adapter {
    private static List<WisdomGuideInfo.ItemsBean> list;
    private static Context context;

    public ScenicSpotSearchResultAdapter(List<WisdomGuideInfo.ItemsBean> list, Context context) {
        this.list = list;
        this.context = context;
    }

    public static void addDatas(List<WisdomGuideInfo.ItemsBean> data){
        if (list == null){
            list = data;
        } else {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder viewHolder, int i) {
        ...
        String lineName = list.get(i).getScenicName();
        lineName = lineName.replace("<em>", "<font color='#24A4FF'>");
        lineName=lineName.replace("</em>", "</font>");
        ((ViewHolderItem) viewHolder).tvTitle.setText(Html.fromHtml(lineName));
        ...
        public ViewHolderItem(@NonNull View itemView) {
            ...
            itemView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ((ScenicSpotSearchResultActivity) context).OnClickListener(list.get(getAdapterPosition()),2);
                }
            });
        }
    }
