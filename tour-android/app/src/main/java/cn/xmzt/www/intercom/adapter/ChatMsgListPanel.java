...
import cn.xmzt.www.intercom.api.model.user.UserInfoObserver;
...
import cn.xmzt.www.nim.uikit.business.robot.parser.elements.group.LinkElement;
import cn.xmzt.www.nim.uikit.business.session.activity.VoiceTrans;
import cn.xmzt.www.nim.uikit.business.session.audio.MessageAudioControl;
import cn.xmzt.www.nim.uikit.business.session.helper.MessageHelper;
...
import cn.xmzt.www.nim.uikit.business.session.viewholder.robot.RobotLinkView;
...
import cn.xmzt.www.nim.uikit.common.ui.recyclerview.adapter.IRecyclerView;
import cn.xmzt.www.nim.uikit.common.ui.recyclerview.listener.OnItemClickListener;
import cn.xmzt.www.nim.uikit.common.ui.recyclerview.loadmore.MsgListFetchLoadMoreView;
import cn.xmzt.www.nim.uikit.common.util.media.BitmapDecoder;
import cn.xmzt.www.nim.uikit.common.util.sys.ClipboardUtil;
...
public class ChatMsgListPanel {
    ...
    private List<IMMessage> items; // 消息数据集合
    private MsgAdapter adapter; // 消息数据adapter
    private ChatMsgPromptPanel incomingMsgPrompt; // 新消息到达提醒
    private Handler uiHandler; // ui变化
    private boolean recordOnly; // 仅显示消息记录，不接收和发送消息
    private boolean remote; // 从服务器拉取消息记录
    private VoiceTrans voiceTrans; // 语音转文字
    private IMMessage forwardMessage; // 待转发消息
    ...
    public ChatMsgListPanel(TeamRoomActivity activity, TeamRoomViewModel viewModel, ActivityTeamRoomBinding dataBinding, Container container) {
        this(activity, viewModel, dataBinding, container, null, false, false);
    }

    public ChatMsgListPanel(TeamRoomActivity activity, TeamRoomViewModel viewModel, ActivityTeamRoomBinding dataBinding, Container container, IMMessage anchor, boolean recordOnly, boolean remote) {
    ...
    private void init(IMMessage anchor) {
        ...
        if (!recordOnly) {
            incomingMsgPrompt = new ChatMsgPromptPanel(container.activity, dataBinding.messageActivityListViewContainer, dataBinding.messageListView, adapter, uiHandler);
        }
    ...
    private void initListView(IMMessage anchor) {
        // RecyclerView
        dataBinding.messageListView.setLayoutManager(new LinearLayoutManager(container.activity));
        dataBinding.messageListView.requestDisallowInterceptTouchEvent(true);
        dataBinding.messageListView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
        ...
        dataBinding.messageListView.setOverScrollMode(View.OVER_SCROLL_NEVER);

        // adapter
        items = new ArrayList<>();
        adapter = new MsgAdapter(dataBinding.messageListView, items, container);
        ...
        initFetchLoadListener(anchor);
        dataBinding.messageListView.setAdapter(adapter);
        dataBinding.messageListView.addOnItemTouchListener(listener);
    }
    ...
    private void doScrollToBottom() {
        dataBinding.messageListView.scrollToPosition(adapter.getBottomDataPosition());
    }
    ...
    public void onIncomingMessage(List<IMMessage> messages) {
        ...
        for (IMMessage message : messages) {
            ...
            // 接收判断是否是
            if (shouldFilterTip(message)){
                setIntercomMessageTip(message);
            }
        }
        if (needRefresh) {
            sortMessages(items);
    ...
    private boolean isLastMessageVisible() {
        LinearLayoutManager layoutManager = (LinearLayoutManager) dataBinding.messageListView.getLayoutManager();
...
//                    NIMClient.getService(MsgService.class).revokeMessage(item).setCallback(new RequestCallback<Void>() {
    ...
    /** ************************************ 判断消息(提示消息) *************************************** */
    private static boolean shouldFilterTip(IMMessage message) {
        ...
        return message != null // 表示不是空的消息
                ...
                && message.getMsgType() == MsgTypeEnum.tip; // 表示获取的消息类型 是 提示消息
    }

    // 检查是否为当前群内的自定放提示消息,广播 是就发送事件
    private void setIntercomMessageTip(IMMessage message){
        // 判断附近消息
        if (message.getRemoteExtension() != null && message.getRemoteExtension().size() > 0){
            ...
            if (!TextUtils.isEmpty(MsgExtensionType.groupId)){
                ...
                if (groupId.equals(MsgExtensionType.groupId)){
                    ...
                    else if (type.equals(MsgExtensionType.Extension_Type_302)){
                        // 收到结束广播通知发送
