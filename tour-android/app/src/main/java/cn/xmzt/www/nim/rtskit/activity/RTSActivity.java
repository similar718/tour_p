package cn.xmzt.www.nim.rtskit.activity;
...
import cn.xmzt.www.nim.rtskit.common.activity.NimToolBarOptions;
import cn.xmzt.www.nim.rtskit.common.activity.ToolBarOptions;
import cn.xmzt.www.nim.rtskit.common.activity.UI;
import cn.xmzt.www.nim.rtskit.common.dialog.EasyAlertDialog;
import cn.xmzt.www.nim.rtskit.common.dialog.EasyAlertDialogHelper;
import cn.xmzt.www.nim.rtskit.common.imageview.HeadImageView;
import cn.xmzt.www.nim.rtskit.common.util.ScreenUtil;
import cn.xmzt.www.nim.rtskit.doodle.DoodleView;
import cn.xmzt.www.nim.rtskit.doodle.SupportActionType;
import cn.xmzt.www.nim.rtskit.doodle.Transaction;
import cn.xmzt.www.nim.rtskit.doodle.TransactionCenter;
import cn.xmzt.www.nim.rtskit.doodle.action.MyPath;
import cn.xmzt.www.nim.rtskit.doodle.constant.ActionTypeEnum;
...
public class RTSActivity extends UI implements View.OnClickListener {
    ...
    public static void incomingSession(Context context, RTSData data, int source) {
        ...
        if (isBusy) {
            ...
            ToastUtils.showText(context, "close session");
            return;
        }
    ...
    private void incoming() {
        ...
        ToastUtils.showText(RTSActivity.this, "incoming session, extra=" + sessionInfo.getExtra());
        ...
        public void onEvent(RTSCalleeAckEvent rtsCalleeAckEvent) {
            if (rtsCalleeAckEvent.getEvent() == RTSEventType.CALLEE_ACK_AGREE) {
                ...
                if (!rtsCalleeAckEvent.isTunReady()) {
                    ToastUtils.showText(RTSActivity.this, "通道开启失败!请查看LOG");
                    return;
                }
                acceptView(); // 进入会话界面
            } else if (rtsCalleeAckEvent.getEvent() == RTSEventType.CALLEE_ACK_REJECT) {
                ToastUtils.showText(RTSActivity.this, R.string.callee_reject);
                onFinish(false);
            }
        }
    };
    ...
    private Observer<RTSCommonEvent> endSessionObserver = new Observer<RTSCommonEvent>() {
        ...
        public void onEvent(RTSCommonEvent rtsCommonEvent) {
            ToastUtils.showText(RTSActivity.this, R.string.target_has_end_session);
            onFinish(false);
        }
    };
            ...
            if (rtsOnlineAckEvent.getClientType() != ClientType.Android) {
                ...
                if (client != null) {
                    String option = rtsOnlineAckEvent.getEvent() == RTSEventType.CALLEE_ONLINE_CLIENT_ACK_AGREE ?
                            "接受" : "拒绝";
                    ToastUtils.showText(RTSActivity.this, "白板演示已在" + client + "端被" + option);
                } else {
                    ToastUtils.showText(RTSActivity.this, "白板演示已在其他端处理");
                }
                onFinish();
            }
        }
    ...
    private Observer<RTSControlEvent> controlObserver = new Observer<RTSControlEvent>() {
        ...
        public void onEvent(RTSControlEvent rtsControlEvent) {
            ToastUtils.showText(RTSActivity.this, rtsControlEvent.getCommandInfo());
        }
    };
    ...
    private RTSChannelStateObserver channelStateObserver = new RTSChannelStateObserver() {
        ...
        public void onConnectResult(String sessionId, RTSTunnelType tunType, long channelId, int code, String file) {
            try {
                ToastUtils.showText(RTSActivity.this, "onConnectResult, tunType=" + tunType.toString() +
                        ", channelId=" + channelId +
                        ", code=" + code + ", file=" + file);
            } catch (Exception e) {
                e.printStackTrace();
        ...
        public void onChannelEstablished(String sessionId, RTSTunnelType tunType) {
            try {
                ToastUtils.showText(RTSActivity.this, "onCallEstablished,tunType=" + tunType.toString());
            } catch (Exception e) {
                e.printStackTrace();
            }
        ...
        public void onDisconnectServer(String sessionId, RTSTunnelType tunType) {
            ToastUtils.showText(RTSActivity.this, "onDisconnectServer, tunType=" + tunType.toString());
            if (tunType == RTSTunnelType.DATA) {
                // 如果数据通道断了，那么关闭会话
                ToastUtils.showText(RTSActivity.this, "TCP通道断开，自动结束会话");
                endSession();
        ...
        public void onError(String sessionId, RTSTunnelType tunType, int code) {
            ToastUtils.showText(RTSActivity.this, "onError, tunType=" + tunType.toString() + ", error=" + code);
            endSession();
        }
    ...
    private Observer<RTSTimeOutEvent> timeoutObserver = new Observer<RTSTimeOutEvent>() {
        ...
        public void onEvent(RTSTimeOutEvent rtsTimeOutEvent) {
            ToastUtils.showText(RTSActivity.this,
...
//        RTSManager.getInstance().setNetworkProxy(networkProxy);
        ...
        sessionId = RTSManager.getInstance().start(account, types, options, notifyOption, new RTSCallback<RTSData>() {
            ...
            public void onFailed(int code) {
                if (code == 11001) {
                    ToastUtils.showText(RTSActivity.this, "无可送达的被叫方");
                } else {
                    ToastUtils.showText(RTSActivity.this, "发起会话失败,code=" + code);
                }
                onFinish();
            }
            ...
            public void onException(Throwable exception) {
                ToastUtils.showText(RTSActivity.this, "发起会话异常,e=" + exception.toString());
                onFinish();
            }
        });

        if (sessionId == null) {
            ToastUtils.showText(RTSActivity.this, "发起会话失败!");
            onFinish();
        }
    }
    ...
    private void acceptSession() {
        ...
        RTSManager.getInstance().accept(sessionId, options, new RTSCallback<Boolean>() {
            ...
            public void onFailed(int code) {
                if (code == -1) {
                    ToastUtils.showText(RTSActivity.this, "接受会话失败,音频通道同时只能有一个会话开启");
                } else {
                    ToastUtils.showText(RTSActivity.this, "接受会话失败, code=" + code);
                }
                onFinish();
            }
            ...
            @Override
            public void onException(Throwable exception) {
                ToastUtils.showText(RTSActivity.this, "接受会话异常, e=" + exception.toString());
                onFinish();
            }
        });
    }
    ...
    private void endSession() {
        RTSManager.getInstance().close(sessionId, new RTSCallback<Void>() {
            ...
            @Override
            public void onFailed(int code) {
                ToastUtils.showText(RTSActivity.this, "挂断请求错误，code：" + code);
            }
    ...
    private void audioSwitch() {
        ...
        RTSManager.getInstance().sendControlCommand(sessionId, content, new RTSCallback<Void>() {
            ...
            public void onSuccess(Void aVoid) {
                ...
                ToastUtils.showText(RTSActivity.this, tip);
            }

            @Override
            public void onFailed(int code) {
                ToastUtils.showText(RTSActivity.this, "控制协议发送失败, code =" + code);
            }

            @Override
            public void onException(Throwable exception) {
                ToastUtils.showText(RTSActivity.this, "控制协议发送异常, e=" + exception.toString());
            }
        });
    }
