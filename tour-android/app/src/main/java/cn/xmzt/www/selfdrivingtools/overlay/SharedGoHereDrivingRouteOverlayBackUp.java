package cn.xmzt.www.selfdrivingtools.overlay;
 ...
 * 导航路线图层类。
 */
public class SharedGoHereDrivingRouteOverlayBackUp extends RouteOverlay{
    ...
    private boolean mIsAddStartEndMaker = false;
     ...
     */
    public SharedGoHereDrivingRouteOverlayBackUp(Context context, AMap amap, AMapNaviPath path,
                                                 NaviLatLng start, NaviLatLng end, List<LatLonPoint> throughPointList, boolean mIsAddStartEndMaker) {
    	super(context);
    	mContext = context; 
        mAMap = amap; 
        this.drivePath = path;
        startPoint = AMapUtil.convertToLatLng(new LatLonPoint(start.getLatitude(),start.getLongitude()));
        endPoint = AMapUtil.convertToLatLng(new LatLonPoint(end.getLatitude(),end.getLongitude()));
        this.throughPointList = throughPointList;
        this.mIsAddStartEndMaker = mIsAddStartEndMaker;
    }
...
//            List<DriveStep> drivePaths = drivePath.getSteps();
            List<AMapNaviStep> drivePaths = drivePath.getSteps();
            for (AMapNaviStep step : drivePaths) {
                List<NaviLatLng> latlonPoints = step.getCoords();
                addDrivingStationMarkers(step, convertToLatLng(new LatLonPoint(latlonPoints.get(0).getLatitude(),latlonPoints.get(0).getLongitude())));
                for (NaviLatLng latlonpoint : latlonPoints) {
                	mPolylineOptions.add(convertToLatLng(new LatLonPoint(latlonpoint.getLatitude(),latlonpoint.getLongitude())));
                	mLatLngsOfPath.add(convertToLatLng(new LatLonPoint(latlonpoint.getLatitude(),latlonpoint.getLongitude())));
				}
            }
            if (startMarker != null) {
                startMarker.remove();
            ...
            if (mIsAddStartEndMaker) {
                addStartAndEndMarker(); // 不想要开始和结尾的marker
            }
...
//        mPolylineOptionscolor.setUseTexture(true);
     ...
     * @param latLng
     */
    private void addDrivingStationMarkers(/*DriveStep driveStep,*/AMapNaviStep step, LatLng latLng) {
...
//                .anchor(0.5f, 0.5f).icon(getDriveBitmapDescriptor()));
        ...
        addStationMarker(new MarkerOptions()
                .position(latLng)
                .title("\u65B9\u5411:" + step.toString()
                        + "\n\u9053\u8DEF:" + step.toString())
                .snippet(step.toString()).visible(nodeIconVisible)
                .anchor(0.5f, 0.5f).icon(getDriveBitmapDescriptor()));
    }
